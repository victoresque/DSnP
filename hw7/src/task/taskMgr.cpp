...
void TaskMgr::add(size_t nMachines) {
    ...
    for (size_t i = 0; i < nMachines;) {
        ...
        if (_taskHash.insert(task)) {
            _taskHeap.insert(task);
            cout << "Task node inserted: " << task << endl;
            i++;
        }
    }
}
...
bool TaskMgr::assign(size_t l) {
    ...
    TaskNode task = _taskHeap.min();
    _taskHeap.delMin();
    task += l;
    _taskHeap.insert(task);
    _taskHash.update(task);

...
class HashMap {
   ...
   public:
    ...
    bool count(const HashKey& k) const {
        size_t bn = bucketNum(k);
        vector<HashNode>& bucket = _buckets[bn];

        for (int i = 0; i < bucket.size(); i++)
            if (k == bucket[i].first) return true;

        return false;
    }
    HashData& operator[](const HashKey& k) {
        size_t bn = bucketNum(k);
        vector<HashNode>& bucket = _buckets[bn];

        for (int i = 0; i < bucket.size(); i++)
            if (k == bucket[i].first) return bucket[i].second;

        _buckets[bn].push_back(make_pair(k, HashData()));
        return _buckets[bn].back().second;
    }

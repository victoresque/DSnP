...
// FIXME: #define toSizeT(t)
#define toSizeT(t)      (((t+SIZE_T-1)/SIZE_T)*SIZE_T)
//#define toSizeT(t)      ((1+(t-1)/SIZE_T)*SIZE_T)
//
// To demote 't' to the nearest multiple of SIZE_T
// e.g. Let SIZE_T = 8;  downtoSizeT(9) = 8, downtoSizeT(100) = 96
// DONE: #define downtoSizeT(t)
...
class MemBlock
{
    ...
    bool getMem(size_t t, T*& ret) {
        // TOCHK: (1) bool getMem(size_t t, T*& ret)

        size_t promoted_t = toSizeT(t);
        if( promoted_t>getRemainSize() ) return false;
...
class MemRecycleList
{
    ...
    T* popFront() {
        // DONE: T* popFront()
        T* _originalFirst = _first;
        if(_first){
            _first = getNext(_first);
        }
        return _originalFirst;
    }
    // push the element 'p' to the beginning of the recycle list
    void pushFront(T* p) {
        // DONE: void pushFront(T* p)
        if(sizeof(T)<sizeof(T*)){
            cerr << "illegal pushFront" << endl;
            return;
        }

        T* originalFirst = _first;
        _first = p;
        setNext(_first, originalFirst);
    }
    ...
    T* getNext(T* p) const {
        // DONE: [NOT reinterpret_cast] T* getNext(T* p) const
        T* pp;
        memcpy((void*)&pp, (void*)p, sizeof(T*));
        return pp;
    }
    void setNext(T*& p, T* pNext) {
        // DONE: [NOT reinterpret_cast] void setNext(T* p, T* pNext)
        memcpy((void*)p, (void*)&pNext, sizeof(T*));
    }
    //
...
private:
    ...
    size_t getArraySize(size_t t) const {
        assert(t % SIZE_T == 0);
        assert(t >= S);
        // TOCHK: (1) size_t getArraySize(size_t t) const
        return (t-sizeof(size_t))/S;
    }
    ...
    MemRecycleList<T>* getMemRecycleList(size_t n) {
        size_t m = n % R_SIZE;
        // TOCHK: (1) MemRecycleList<T>* getMemRecycleList(size_t n)
        ...
        MemRecycleList<T>* p2 = new MemRecycleList<T>(n);
        p->setNextList(p2);
        return p2;
    }
    ...
    T* getMem(size_t t) {
        ...
        bool foundRecycleList = false;
        size_t arraySize = getArraySize(t);
        MemRecycleList<T>* p_rl = &_recycleList[arraySize%R_SIZE];
        
        while(p_rl){
            if( p_rl->getArrSize()==arraySize && p_rl->_first!=NULL ){
                foundRecycleList = true;
                ret = p_rl->popFront();
                break;
            }
            p_rl = p_rl->_nextList;
        ...
        if(foundRecycleList){
            #ifdef MEM_DEBUG
            cout << "Recycled from _recycleList[" << getArraySize(t) << "]..." << ret << endl;
            #endif // MEM_DEBUG
            return ret;
        }
        ...
        if(!_activeBlock->getMem(t, ret)){
            ...
            assert(_activeBlock->getMem(rs, ret));

            if(rs>=S){
                rs = downtoSizeT(rs);
                #ifdef MEM_DEBUG
                cout << "Recycling " << ret << " to _recycleList[" << getArraySize(rs) << "]\n";
                #endif // MEM_DEBUG
                getMemRecycleList(getArraySize(rs))->pushFront(ret);
            }
    ...
    size_t getNumBlocks() const {
        // DONE: size_t getNumBlocks() const
        size_t cnt = 0;
